% Conducts a position analysis of the inverted slider-crank linkage and
% plots the positions of points B, C and P
...
if ((a + d) < c*sin(delta))
  ...
  if (abs(a - d) >= c*sin(delta))
    ...
    theta2min = acos((a^2 + d^2 - (c*sin(delta))^2)/(2*a*d));
...
title('Paths of points B, C and P on Inverted Slider-Crank')
xlabel('x-position [m]')
ylabel('y-position [m]')
legend('Point B', 'Point C', 'Point P','Location','SouthEast') 

% Save the plot
saveas(gcf, 'InvSlider_Position_Analysis - plot.png')