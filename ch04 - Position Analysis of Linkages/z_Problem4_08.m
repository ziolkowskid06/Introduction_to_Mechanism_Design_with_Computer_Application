% Problem 4.5 (threebar crank-slider linkage)
...
% Linkage dimensions
a = 2;         % crank length (m)
d = 4;         % horizontal length between ground pins (m)
e = 3;         % vertical length between ground pins (m)
p = 5;         % slider length (m)
...
for i = 1:N
  ...
  theta3(i) = atan2(-a*sin(theta2(i))+e, d - a*cos(theta2(i)));
...
text(       xA(1),        xA(2)-0.2, 'A', 'HorizontalAlignment', 'center');
text(xB(1,iTheta), xB(2,iTheta)+0.2, 'B', 'HorizontalAlignment', 'center');
text(       xD(1),        xD(2)+0.2, 'D', 'HorizontalAlignment', 'center');
text(xP(1,iTheta), xP(2,iTheta)+0.2, 'P', 'HorizontalAlignment', 'center');
...
% Save the plot
saveas(gcf, 'z_Problem4_08 - plot.png')