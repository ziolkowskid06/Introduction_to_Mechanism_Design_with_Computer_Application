...
% Linkage dimensions
a = 0.065;           % crank length (m)
b = 0.095;           % coupler 1 length (m)
c = 0.095;           % coupler 2 length (m)
d = 0.080;           % distance between ground pins (m)
u = 0.065;           % length of link on gear 2 (m)
N1 = 50;             % number of teeth on gear 1
N2 = 25;             % number of teeth on gear 2
rho = N1/N2;         % gear ratio
phi = 180*pi/180;    % offset angle between gears
gamma3 =  30*pi/180; % angle to point P on coupler 1 (CCW rotation)
gamma4 = -30*pi/180; % angle to point Q on coupler 2 (CW rotation)
p = 0.070;           % distance to point P on coupler 1
q = 0.070;           % distance to point Q on coupler 2
...
hold on
plot(xP(1,:),xP(2,:), 'Color', [204/255 102/255  0/255],'Linewidth',1.5)
plot(xQ(1,:),xQ(2,:), 'Color', [  0/255 153/255 76/255],'Linewidth',1.5)
axis equal
grid on
...
text(  xB(1,iTheta)-.005,       xB(2,iTheta),'B','HorizontalAlignment','center');
text(  xC(1,iTheta)+.005,       xC(2,iTheta),'C','HorizontalAlignment','center');
text(              xD(1),         xD(2)-.005,'D','HorizontalAlignment','center');
text(xE_l(1,iTheta)+.005,xE_l(2,iTheta)+.005,'E','HorizontalAlignment','center');
text(  xP(1,iTheta)+.005,  xP(2,iTheta)+.005,'P','HorizontalAlignment','center');
text(  xQ(1,iTheta)+.005,       xQ(2,iTheta),'Q','HorizontalAlignment','center');
axis equal
grid on
title('Paths of points E, P and Q on Fivebar Linkage')
xlabel('x-position [m]')
ylabel('y-position [m]')
legend('Point E ', 'Point P', 'Point Q','Location','SouthEast')

% Save the plot
saveas(gcf, 'z_Problem4_19 - plot.png')