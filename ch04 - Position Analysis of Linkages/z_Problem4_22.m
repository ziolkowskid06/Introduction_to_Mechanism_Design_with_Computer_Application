% Problem 4.22 (sixbar stephenson type I linkage)
...
p = 0.150;            % AE length to third pin on crank triangle (m)
q = 0.150;            % DF length to third pin on rocker triangle (m)
u = 0.120;            % EG length of link 5 (m)
v = 0.160;            % FG length of link 6 (m)
z = 0.100;            % FP length of link 7 (m)
gamma2 =  20*pi/180;  % internal angle of crank triangle (CCW rotation)
gamma3 = -50*pi/180;  % internal angle of triangle FGP (CW rotation) 
...
[xB,xC,xE,xF,xG,xP]                  = deal(zeros(2,N)); % points
...
for i = 1:N
  ...
  [eFP,nFP] = UnitVector(theta6(i) + gamma3);
                 
  % Calculate position of point G
  xG(:,i) = FindPos(xE(:,i), u, e5);
  xP(:,i) = FindPos(xF(:,i), z, eFP);
end
...
plot(xF(1,:), xF(2,:), 'Color', cGre(6,:))
plot(xG(1,:), xG(2,:), 'Color', cBlk(3,:))
plot(xP(1,:), xP(2,:), 'Color', cBlk(8,:))
...
patch([xD(1) xF(1,iTheta) xC(1,iTheta)],...
      ...
      'EdgeColor',cBlk(6,:),'LineWidth',2,'FaceAlpha',0.5); % Make the links semitransparent
patch([xF(1,iTheta) xG(1,iTheta) xP(1,iTheta)],...
      [xF(2,iTheta) xG(2,iTheta) xP(2,iTheta)],cGre(8,:),...
      'EdgeColor',cBlk(6,:),'LineWidth',2,'FaceAlpha',0.5); % Make the links semitransparent
...
% Plot joints on linkage
 plot([xB(1,iTheta) xC(1,iTheta) xE(1,iTheta) xF(1,iTheta) xG(1,iTheta) xP(1,iTheta)],...
      [xB(2,iTheta) xC(2,iTheta) xE(2,iTheta) xF(2,iTheta) xG(2,iTheta) xP(2,iTheta)],...
...
text(xP(1,iTheta),xP(2,iTheta)+0.010,'P','HorizontalAlignment','center');

% Plot parameters
axis equal
grid on
title('Paths of points E, F, G and P on Stephenson Type I sixbar linkage')
xlabel('x-position [m]')
ylabel('y-position [m]')
legend('Point E', 'Point F', 'Point G', 'Point P','Location','SouthEast')

% Save the plot
saveas(gcf, 'z_Problem4_22 - plot.png')