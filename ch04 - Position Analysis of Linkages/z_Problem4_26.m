% Problem 4.26 (sixbar Watt type II linkage)
...
gamma3 =  50*pi/180; % internal angle of triangle EGP (CW rotation)
...
% Plot trajectories of points C, E, G, and P 
figure; hold on
...
patch([x0(1) xD(1) xF(1)],...
      ...
      'EdgeColor',cBlk(6,:),'LineWidth',2,'FaceAlpha',0.5); % Make the links semitransparent
patch([xE(1,iTheta) xP(1,iTheta) xG(1,iTheta)],...
      [xE(2,iTheta) xP(2,iTheta) xG(2,iTheta)],cGre(10,:),...
      'EdgeColor',cBlk(6,:),'LineWidth',2,'FaceAlpha',0.5); % Make the links semitransparent
...
% Plot joints on linkage
 plot([xB(1,iTheta) xC(1,iTheta) xE(1,iTheta) xG(1,iTheta) xP(1,iTheta)],...
      [xB(2,iTheta) xC(2,iTheta) xE(2,iTheta) xG(2,iTheta) xP(2,iTheta)],...
...
text(xP(1,iTheta),xP(2,iTheta)+0.012,'P','HorizontalAlignment','center');


% Plot parameters
axis equal
grid on
title('Paths of points C, E, G, and P on Watt Type II sixbar linkage')
xlabel('x-position [m]')
ylabel('y-position [m]')
legend('Point E', 'Point C', 'Point G', 'Point P', 'Location','SouthEast')

% Save the plot
saveas(gcf, 'z_Problem4_26 - plot.png')