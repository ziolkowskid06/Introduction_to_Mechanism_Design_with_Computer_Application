% Problem 5.08 (Inverted Slider-Crank linkage)
...
% Linkage dimensions
a = 0.060;           % crank length (m)
c = 0.080;           % rocker length (m)
d = 0.150;           % length between ground pins (m)
p = 0.120;           % slider length (m)
delta = 80*pi/180;   % angle between slider and rocker (converted to rad)
...
[vC_mag, vP_mag] = deal(zeros(1,N));       % allocate space for velocities  
...
% Velocities magnitudes 
  vP_mag(i) = sqrt(vP(1,i).^2+vP(2,i).^2);
  vC_mag(i) = sqrt(vC(1,i).^2+vC(2,i).^2);  
...
figure
plot(theta2*180/pi,vP_mag,'Color',[153/255 153/255 153/255])
hold on
plot(theta2*180/pi,vC_mag,'Color',[0/255 153/255 76/255])
legend('vC','vP','Location','Southeast')
title('Velocity of points C and P on Inverted Slider-Crank Linkage')
...
% Save the plot
saveas(gcf, 'z_Problem5_08 - plot.png')